...
const Bugs = () => {
  ...
  if (tasks) {
    ...
    currentTask =
      tasks.length > 0 &&
      tasks.filter((task) => task.taskNumber === tasknumber);
    ...
    <div className="bugs-list">
      <form className=" bug-form" onSubmit={handleSubmit}>
        ...
        <button
          ...
          onClick={() => setTimeout(onClear, 500)}
        >
      ...
      <div className="bug-list">
        <div>
          <h2 className="page-title">
            Bugs List for task :{" "}
            <Link
              to={`https://jira-prd/browse/${tasknumber.toUpperCase()}`}
              target="_blank"
            >
        ...
        {currentTaskId && (
          <BugList currentTaskId={currentTaskId} tasksNumbers={tasksNumbers} />
        )}
      </div>
    </div>
  );
