...
const Bug = ({ bug, currentTask }) => {
  ...
  return (
    <React.Fragment key={bug[0].id}>
      {bug && (
        <div className="bug-item-summary">
          <div className="bug-item-author">
            <p className="bug-item-summary-created">
              Created by:{" "}
              <span style={{ textTransform: "capitalize" }}>
                {bug[0].createdBy.displayName}
              </span>
            </p>
            <p className="bug-item-summary-created">
              Created at:{" "}
              {formatDistanceToNow(bug[0].createdAt.toDate(), {
                addSuffix: true,
              })}
            </p>
          </div>
          ...
          <h2>Description</h2>
          <p className="bug-item-summary-description">
            {bug[0].bugDescription}
          </p>
          <p className="bug-item-summary-market ">
            <span className="bold">Market:</span>{" "}
            <span className="blue">
              {bug[0].marketList.map((m, index) => (
                <span key={m.marketName} style={{ marginRight: "10px" }}>
                  {m.marketName} {index !== bug[0].marketList.length - 1 && "|"}
                </span>
              ))}
            </span>{" "}
          </p>
          <p>
            ...
            {bug[0].platformList.map((b, index) => (
              <span key={b.platformName} style={{ marginRight: "10px" }}>
                {b.platformName}{" "}
                {index !== bug[0].platformList.length - 1 && "|"}
              </span>
            ))}
          </p>
          <p>
          ...
          <Link to={`/task/${bug[0].tasknumber}`}>
            <TbArrowBackUp className="back-icon"></TbArrowBackUp> Back to Bugs
            List
          </Link>
