...
const BugList = ({ currentTaskId }) => {
  ...
  const updateTaskStatus = async () => {
    ...
    const allStatusEqual = bugStatusArray.every(
      (status) => status === "Fixed-verified" || status === "Ignored"
    );
  ...
  return (
    <>
      <div className="btn-container-bug-list">
        <Link to={`/`} className="back-to-tasklist">
          {" "}
          <TbArrowBackUp className="back-icon"></TbArrowBackUp> Back to Task
          List
        </Link>
        <button className="btn btn-bug-list" onClick={() => updateTaskStatus()}>
          ...
          <span className="label">
            Change Task Status: {task.status.toUpperCase()}
          </span>
        </button>
      </div>
