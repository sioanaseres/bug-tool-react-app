...
const BugSummary = ({ bug, tasknumber, index, task }) => {
  ...
  return (
    <div
      className={
        bug.status &&
        (bug.status === "Fixed-verified" || bug.status === "Ignored")
          ? "fixed-bug bug-summary-item"
          : "bug-summary-item"
      }
    >
      <div className="bug-summary-item-details">
        ...
        <p className="bug-details" key={index * 2000000}>
          ...
          ) : bug.status === "Fixed" || bug.status === "Ignore" ? (
            <AiOutlineCheckSquare className="modified-icon" />
          ) : (
            <SiTodoist className="todo-icon" />
          )}
        ...
        <p
          className={
            bug.severity && bug.severity === "High-priority"
              ? "error bug-severity"
              : "bug-severity"
          }
        >
        ...
        <p className="bug-market">
          {bug.marketList.map((m, index) => (
            <span key={m.marketName} className="market-item">
              {m.marketName} {index !== bug.marketList.length - 1 && "|"}{" "}
            </span>
          ))}
        </p>
        <p className="bug-environemt">{bug.environment}</p>
        <p className="bug-platform">
          {" "}
          {bug.platformList.map((m, index) => (
            <span key={m.platformName} className="platform-item">
              {m.platformName} {index !== bug.platformList.length - 1 && "|"}{" "}
            </span>
          ))}
        </p>
